...
def run_fine_tuning(
    ...
    data_stats = {
        ...
        "has_system_message": any("system" in msg["role"] for sample in training_data for msg in sample["messages"]),
    ...
    # Save JSON log
    json_filename = f"fine_tuning_job_{job.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    if job.status == "succeeded":
        print(f"Fine-tuning completed successfully! New model ID: {job.fine_tuned_model}")
    else:
